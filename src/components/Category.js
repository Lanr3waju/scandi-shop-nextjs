  ...
  return (
    <ul className="flex flex-wrap justify-start gap-20 pb-5 min-h-screen">
      {store?.products.map(({ id, gallery, name, prices, inStock }) => (
        <li key={id} className="w-80 h-96 shadow-sm rounded-lg p-6 shadow-hoverBackground hover:shadow-overlay hover:shadow-lg transition-shadow relative">
          <span className={`text-3xl font-medium text-overlay absolute top-1/4 left-[18%] -rotate-45 opacity-100 z-20 ${inStock && 'hidden'}`}>Out of stock</span>
          <Link className={`${!inStock && 'opacity-40'}`} href={`/${id}`}>
            <Image
              ...
              alt={name}
            />
            <h2 className="opacity-80 my-3">{name}
              {prices.map((price) => {
