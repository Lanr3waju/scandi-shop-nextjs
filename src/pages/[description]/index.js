...
export default function DescriptionPage() {
  ...
  const product = store.data.categories[0].products.find((product) => product.id === router.query.description);
              ...
              product?.gallery.map((image) => (
                <li key={image}>
                  <button className="mb-4" onClick={() => setActiveImage(image)}>
                    <Image placeholder="blur" blurDataURL="/small-placeholder.png" className="w-24 h-24 object-contain" src={image} alt='product' width={96} height={96} />
                  </button>
                </li>
              ))
            }
